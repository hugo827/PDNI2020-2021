...
void createDatasets(void) {
	...
	char chaine[] = "movement, gender, id, accelleration, accelleration, accelleration, ..., acceleration\n";
	...
	// Ouverture du fichier data_subjects_info.csv et test
	fopen_s(&fiDsi, DSI, "r");

	if (&fiTrainSet != NULL && &fiTestSet != NULL && &fiDsi != NULL) {

		// Écriture de la ligne la premier ligne (titre)
		fwrite(&chaine, sizeof(chaine), 1, fiTrainSet);
		fwrite(&chaine, sizeof(chaine), 1, fiTestSet);
		...
		for (int iFolder = 0; iFolder < NB_FOLDER; iFolder++) {
			...
			for (int iFile = 1; iFile <= NB_FILE; iFile++) {
				...
				else
				{
					...
					while (fgetc(fiAFile) != '\n');
					snprintf(write, 16, "%d,%d,%d,", getMovement(iFolder), getGender(fiDsi), iFile);
					if (fileInTestSet) {
					...
					iLineSub = 0;
					while ((!feof(fiAFile)) && (iLineSub < 600)) {
						...
						if (fileInTestSet) {
							fwrite(&write, 10, 1, fiTestSet);
						}
						else {
							fwrite(&write, 10, 1, fiTrainSet);
						}
						iLineSub++;
					}
					snprintf(write, 2, "%c", '\n'); // Une fois le fichier sub_x lu, il est nécessaire de passer à la ligne dans le fichier de dataset correspondant
					if (fileInTestSet) {
						fwrite(&write, 2, 1, fiTestSet);
					}
					else {
						fwrite(&write, 2, 1, fiTrainSet);
					}
					fclose(fiAFile);
				}
			}
