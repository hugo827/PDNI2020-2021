...
void testSetEvaluation(void) {
	...
	if (fiTestSet != NULL && fiModel != NULL) {
		...
		while (!feof(fiTestSet) && iLigne < nbTests) {
			...
			mvt = valueFromModel(fiTestSet);
			gender = valueFromModel(fiTestSet);
			id = valueFromModel(fiTestSet);

			while (iVacc < nb_acc) {
				acc = valueFromModel(fiTestSet);
...
void convertFileToTable(FILE* file, Models models[]) {
	...
	while (!feof(file) && lineModel <= NB_MOVEMENTS) {
		iVacc = 1;
		models[lineModel - 1].move = valueFromModel(file);
		while (iVacc <= 601 && fgetc(file) != '\n') {
			models[lineModel - 1].v_acc[iVacc - 1] = valueFromModel(file);
			iVacc++;
		}
		lineModel++;
	}
