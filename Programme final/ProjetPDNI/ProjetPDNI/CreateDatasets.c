...
void createDatasets(void) {
	...
	if (fiTrainSet != NULL && fiTestSet != NULL && fiDsi != NULL) {
		...
		fwrite(&firstLineString, sizeof(firstLineString), 1, fiTrainSet);
		fwrite(&firstLineString, sizeof(firstLineString), 1, fiTestSet);
		...
		for (int iFolder = 0; iFolder < NB_FOLDER; iFolder++) {
			...
			for (int iFile = 1; iFile <= NB_FILE; iFile++) {
				...
				else
				{
					...
					while (fgetc(fiAFile) != '\n');
					snprintf(write, 15, "%d,%d,%d,", getMovement(iFolder), getGender(fiDsi), iFile);
					...
					while ((!feof(fiAFile)) && (iLineSub < NB_VAC_READ)) {
						...
						// 10 : nombres d'octets nécessaires pour le format ci dessus (accélération)
						if (fileInTestSet) {
					...
					// 2 octets nécessaire pour le changement de lignes
					if (fileInTestSet) {
